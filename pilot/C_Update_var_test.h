...
void K_Psi(std::vector<mrcpp::CompFunction<3>> &K_ij_T, std::vector<mrcpp::CompFunction<3>> &K_ij_B, Orbital_Pointer_List Psi_k, MultiResolutionAnalysis<3> &MRA, PoissonOperator &P){
    ...
    for (int i=0; i<6; i++){
        ...
        std::vector<ComplexDouble> Fourier_Coefficients(6, ComplexDouble(1.0, 0.0));
    ...
    std::cout << "K_ij[4] TOP AND BOT = " << K_ij_T[4].getSquareNorm() << '\t' << K_ij_B[4].getSquareNorm() << '\n';
    std::cout << "K_ij[5] TOP AND BOT = " << K_ij_T[5].getSquareNorm() << '\t' << K_ij_B[5].getSquareNorm() << '\n';
    }
...
void J_Psi(std::vector<std::vector<mrcpp::CompFunction<3>*>> &J_Psi_k, Orbital_Pointer_List Psi_k, CompFunction<3> Mean_field_potential, MultiResolutionAnalysis<3> &MRA, PoissonOperator &P){
    ...
    // Variable declaration
    std::vector<FunctionTree<3, double>*> Rho_t(6); // the 6 top components of the density
    std::vector<FunctionTree<3, double>*> Rho_b(6); // the 6 bottom components of the density
    ...
    // I'll need these because make_density_local only works with CompFunction, making them some temporary variables
    std::vector<CompFunction<3>> Rho_t_C(6);
    std::vector<CompFunction<3>> Rho_b_C(6);
    for (int k=0; k<6; k++){
