...
void print_memory_usage() {
    ...
    long size, resident;
    statm >> size >> resident;
    std::cout << "Virtual Memory: " << 0.000001*size * 6 << " GB\t";
    std::cout << "Resident Set Size: " << 0.000001*resident * 6 << " GB\n";
}
...
#include "C_atom_ZORA_utils_test.h"
#include "C_Update_var_test.h"
...
void SCF_Cycle_ZORA(MultiResolutionAnalysis<3> &MRA, 
    ...
    double &norm_diff_max) {
        ...
        Spinor_gradients_pointer_list Spinor_gradients(6, std::vector<std::vector<mrcpp::CompFunction<3>*>>(2, std::vector<mrcpp::CompFunction<3>*>(3, nullptr)));
        for (int i = 0; i < 6; ++i) { //N electrons
            for (int j = 0; j < 2; ++j) { //history
                for (int k = 0; k < 3; ++k) { // space dimensions
...
// ==================================================================================================================================
    ...
    Z = 6;
    n_electrons = Z; // --> As we are dealing with the H type atom
    ...
    std::function<double(const Coord<3> &x)> C_2p = [] (const mrcpp::Coord<3> &r) -> double {
        auto R = std::sqrt(r[0]*r[0] + r[1]*r[1] + r[2]*r[2]);
    ...
    std::vector<mrcpp::CompFunction<3>> Psi_5(2);
    std::vector<mrcpp::CompFunction<3>> Psi_6(2);
    ...
    // 2p1/2 spin up
    project(Psi_5[0], C_2p, building_precision);
    project(Psi_5[1], zero, building_precision);
    //std::cout << "Psi_2c_4[0] norm = " << Psi_4[0].getSquareNorm() << '\n';
    //std::cout << "Psi_2c_4[1] norm = " << Psi_4[1].getSquareNorm() << '\n';
    // 2s spin down
    project(Psi_6[0], zero, building_precision);
    project(Psi_6[1], C_2p, building_precision);
    //std::cout << "Psi_2c_4[0] norm = " << Psi_4[0].getSquareNorm() << '\n';
    //std::cout << "Psi_2c_4[1] norm = " << Psi_4[1].getSquareNorm() << '\n';
    ...
    CompFunction<3> Psi_alt_3(mra, 2);
    CompFunction<3> Psi_alt_4(mra, 2);
    project(Psi_alt_3, Be_1s, building_precision, 0); // Initialize the Psi_alt_ function to zero
    project(Psi_alt_4, Be_2s, building_precision, 1); // Initialize the kramer_1 function to zero

    Psi_vector.push_back(Psi_alt_3);
    Psi_vector.push_back(Psi_alt_4);

    CompFunctionVector Phi_vector;
    CompFunction<3> Psi_alt_5(mra, 2);
    project(Psi_alt_5, Be_2s, building_precision, 1);
    ...
    for (int i = 0; i < overlap.rows(); ++i) {
        for (int j = 0; j < overlap.cols(); ++j) {
            std::cout << overlap(i,j) << " " << dot(Psi_vector[i], Phi_vector[j]) << ", " << '\t';
        }
        std::cout << '\n';
    }
    ...
    Psi_rotated.push_back(Psi_alt_3);
 ...
 */  
    ...
    std::vector<std::vector<mrcpp::CompFunction<3>*>> J_ij(6, std::vector<mrcpp::CompFunction<3>*>(2, nullptr));
    std::vector<std::vector<mrcpp::CompFunction<3>*>> K_ij(6, std::vector<mrcpp::CompFunction<3>*>(2, nullptr));
    std::vector<mrcpp::CompFunction<3>> J_ij_T(6);
    std::vector<mrcpp::CompFunction<3>> J_ij_B(6);
    std::vector<mrcpp::CompFunction<3>> K_ij_T(6);
    std::vector<mrcpp::CompFunction<3>> K_ij_B(6);
