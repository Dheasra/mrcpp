...
ComplexDouble compute_Term2_T_ZORA(MultiResolutionAnalysis<3> &MRA, std::vector<std::vector<mrcpp::CompFunction<3>*>> &Nabla_Psi_2c, mrcpp::CompFunction<3> &K_tree, std::vector<mrcpp::CompFunction<3> *> &Nabla_K_tree,  std::vector<mrcpp::CompFunction<3>> &Psi_2c){
    ...
    for (int i=0;i<3;i++){
        // Psi_Nabla_k_tmp = Psi_2c[0] * Nabla_K_tree[i]
        mrcpp::multiply(Psi_Nabla_k_tmp, Psi_2c[0], *Nabla_K_tree[i],building_precision, false, false, false);   // <---- Here we dereference the pointer
        // Psi_t_Nabla_K[i] = Psi_Nabla_k_tmp
        Psi_t_Nabla_K.push_back(Psi_Nabla_k_tmp);
    }
    
    // Compute Psi_bottom * Nabla_K
    for (int i=0;i<3;i++){
        // Psi_Nabla_k_tmp = Psi_2c[1] * Nabla_K_tree[i]
        mrcpp::multiply(Psi_Nabla_k_tmp, Psi_2c[1], *Nabla_K_tree[i],building_precision, false, false, true);   // <---- Here we dereference the pointer
        // Psi_b_Nabla_K[i] = Psi_Nabla_k_tmp
        Psi_b_Nabla_K.push_back(Psi_Nabla_k_tmp);
    }

    std::vector<ComplexDouble> Kinetic_term_1;

    // < \Psi \Nabla(K) | \Nabla(\Psi) >
    ...
    // You could do first the dot product between \Nabla(K) and \Nabla(\Psi) and then multiply by the Psi, but this would require more memory
...
ComplexDouble compute_Term3_T_ZORA(MultiResolutionAnalysis<3> &MRA, std::vector<std::vector<mrcpp::CompFunction<3>*>> &Nabla_Psi_2c, mrcpp::CompFunction<3> &K_tree, std::vector<mrcpp::CompFunction<3> *> &Nabla_K_tree,  std::vector<mrcpp::CompFunction<3>> &Psi_2c){
    ...
    std::cout << "Old norm of SOC_Psi_t = " << SOC_Psi_t.getSquareNorm() << '\n';
    std::cout << "Old norm of SOC_Psi_b = " << SOC_Psi_b.getSquareNorm() << '\n';
    ...
    std::cout << "New norm of SOC_Psi_t = " << SOC_Psi_t.getSquareNorm() << '\n';
    std::cout << "New norm of SOC_Psi_b = " << SOC_Psi_b.getSquareNorm() << '\n';
...
void compute_term_C_Propagator(MultiResolutionAnalysis<3> &MRA, std::vector<std::vector<mrcpp::CompFunction<3> *>> &Nabla_Psi_2c , std::vector<mrcpp::CompFunction<3> *> &Nabla_K_tree, CompFunction<3> &K_inverse, CompFunction<3> &term_C_top,CompFunction<3> &term_C_bottom){
    ...
    /*
    for (int i = 0; i<3; i++){
        mrcpp::multiply(*K_Nabla_Psi_t[i], Nabla_K_tree, *Nabla_Psi_2c[0][i],building_precision, false, false, false);
        mrcpp::multiply(*K_Nabla_Psi_b[i], Nabla_K_tree, *Nabla_Psi_2c[1][i],building_precision, false, false, false);
    }
    */
    ...
    std::vector<mrcpp::CompFunction<3>*> cross_top(3, new mrcpp::CompFunction<3>(MRA));
    std::vector<mrcpp::CompFunction<3>*> cross_bottom(3, new mrcpp::CompFunction<3>(MRA));
    Cross_Product_Compfunct(cross_top, Nabla_K_tree, Nabla_Psi_2c[0], MRA);
    Cross_Product_Compfunct(cross_bottom, Nabla_K_tree, Nabla_Psi_2c[1], MRA);
...
void Update_Nabla_Psi(MultiResolutionAnalysis<3> &MRA, std::vector<mrcpp::CompFunction<3>> &Psi_2c, std::vector<std::vector<mrcpp::CompFunction<3> *>> &Nabla_Psi_2c){
    ...
    // Gradient of Psi_top * K
    auto Nabla_Psi_t_K = mrcpp::gradient(D,Psi_2c[0]);
    // Gradient of Psi_bottom * K
    auto Nabla_Psi_b_K = mrcpp::gradient(D,Psi_2c[1]);

    // Now we update the Nabla_Psi_2c
    for (int i=0; i<3; i++){
        *Nabla_Psi_2c[0][i] = *Nabla_Psi_t_K[i];
        *Nabla_Psi_2c[1][i] = *Nabla_Psi_b_K[i];
    }
}
